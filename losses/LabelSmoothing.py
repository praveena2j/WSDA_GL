...
class LSR(nn.Module):
	...
	def gaussian(self, mu):
		...
		gauss_func = np.exp(-np.power(x - mu, 2.) / (2 * var))
		gauss_func = np.exp(gauss_func) / np.sum(np.exp(gauss_func), axis=0)
		return torch.from_numpy(gauss_func).float().cuda()


	def SLCDLoss(self, outputs, data_outputs):
		classes = torch.LongTensor([0,1,2,3,4,5]).cuda()
		#data_outputs = torch.FloatTensor(data_outputs)
		#data_outputs = data_outputs.type(torch.FloatTensor).cuda()
		...
		LD = torch.stack(diff).type(torch.FloatTensor).cuda()
	...
	def softlabels(self, labels, classes, value):
		...
		softtarget[0, :] = torch.FloatTensor([0.5, 0.4, 0.09, 0.009, 0.0009, 0.0001])
		softtarget[1, :] = torch.FloatTensor([0.25, 0.4, 0.25, 0.09, 0.009, 0.001])
		softtarget[2, :] = torch.FloatTensor([0.049, 0.25, 0.4, 0.25, 0.049, 0.002])
		softtarget[3, :] = torch.FloatTensor([0.002, 0.049, 0.25, 0.4, 0.25, 0.049])
		softtarget[4, :] = torch.FloatTensor([0.001, 0.009, 0.09, 0.25, 0.4, 0.25])
		softtarget[5, :] = torch.FloatTensor([0.0001, 0.0009, 0.009, 0.09, 0.4, 0.5])
	...
	def forward(self, x, target):
		...
		#slcd_loss = self.SLCDLoss(x, target)
		smoothed_target = self._smooth_label(target, x.size(1), self.e)
		#target = target.view(-1,1)
		#x = self.log_softmax(x)
		#print(x)
		x = torch.log(x)
		#loss = self.kl_loss(x, smoothed_target).sum(1)
		...
		#slcd_loss = slcd_loss * sample_weights.type(loss.type())
		#print(loss)
		#sys.exit()

		if self.reduction == 'none':
			return loss #+ slcd_loss

		elif self.reduction == 'sum':
			return torch.sum(loss) #+ torch.sum(slcd_loss)

		elif self.reduction == 'mean':
			return torch.mean(loss) #+ torch.mean(slcd_loss)
